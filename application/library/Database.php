...
class Database
{
    ...
    private function __construct()
    {
        ...
        $conf = Yaf_Registry::get('config')->get('database');
        $this->_connect($conf);
     ...
     */
    public function query($sql, $fetchType = Database::FETCH_ASSOC, $bolUseResult = false)
    {
        if(!is_string($sql))
        {
            Log::addWarning("Input SQL is not valid: '". $sql ."'");
            return false;
        }
        self::$lastSql = $sql;
        ...
        if(is_bool($res) || $res === NULL)
        {
            ...
            if(!$ret)
            {
                Log::addWarning("MySQL query failed: '". $sql ."'");
            }
        }
        // we have result
        else
    ...
    */
    ...
    private function _connect($conf = NULL)
    {
        ...
        if (!$this->isConnected)
        {
            Log::addErr('connect to MySQL failed');
            exit();
        }
    ...
    */
    private function _getSelect($tables, $fields, $conds = NULL, $options = NULL, $appends = NULL)
    {
        ...
        if($options !== NULL)
        {
            $options = $this->_makeList($options, Database::LIST_COM, ' ');
            if(!strlen($options))
            {
                $this->sql = NULL;
                return NULL;
        ...
        // 2. fields
        $fields = $this->_makeList($fields, Database::LIST_COM);
        if(!strlen($fields))
        {
            $this->sql = NULL;
            return NULL;
        ...
        // 3. from
        $tables = $this->_makeList($tables, Database::LIST_COM);
        if(!strlen($tables))
        {
            $this->sql = NULL;
            return NULL;
        ...
        if($conds !== NULL)
        {
            $conds = $this->_makeList($conds, Database::LIST_AND);
            if(!strlen($conds))
            {
                $this->sql = NULL;
                return NULL;
        ...
        if($appends !== NULL)
        {
            $appends = $this->_makeList($appends, Database::LIST_COM, ' ');
            if(!strlen($appends))
            {
                $this->sql = NULL;
                return NULL;
    ...
    */
    private function _getInsert($table, $row, $options = NULL, $onDup = NULL)
    {
        ...
        // 3. clumns and values
        $row = $this->_makeList($row, Database::LIST_SET);
        if(!strlen($row))
        {
            $this->sql = NULL;
            return NULL;
        ...
        if(!empty($onDup))
        {
            ...
            $onDup = $this->_makeList($onDup, Database::LIST_SET);
            if(!strlen($onDup))
            {
                $this->sql = NULL;
                return NULL;
    ...
    */
    ...
    private function _makeList($arrList, $type = Database::LIST_SET, $cut = ', ')
    {
        ...
        // for where conds
        else if($type == Database::LIST_AND)
        {
            foreach($arrList as $name => $value)
            {
                if(is_int($name))
